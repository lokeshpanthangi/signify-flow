...
const SignFormCard = ({ form }: { form: SignForm }) => (
  <motion.div
  ...
  >
    ...
    <p className="text-xs text-stone-400 font-medium mb-6">Created on {form.dateCreated}</p>
    ...
    <div className="flex items-center justify-between pt-4 border-t border-stone-100">
      ...
      <button
        className="text-sm font-medium text-stone-500 hover:text-green-600 transition-colors flex items-center gap-1"
        onClick={() => {
          // In a real app, this would copy to clipboard or open a modal
          alert(`Share URL: ${window.location.origin}${form.url}`);
        }}
      >
        Share <LinkIcon className="h-3 w-3" />
      </button>
    </div>
  </motion.div>
);
          ...
          {activeTab === 'signforms' && (
            <section>
              <div className="flex items-center justify-between mb-8">
                ...
                <button className="bg-[#1A1C1E] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-black transition-colors flex items-center gap-2 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                {mockSignForms.map((form) => (
                  <SignFormCard key={form.id} form={form} />
                ))}
              </div>
            </section>
          )}
